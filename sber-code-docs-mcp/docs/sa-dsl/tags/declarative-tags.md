...
[ ![](https://developers.sber.ru/docs/img/badges/Code.png)Code ](https://developers.sber.ru/docs/ru/va/code/overview)[ ![](https://developers.sber.ru/docs/img/badges/ChatApp.png)Chat App ](https://developers.sber.ru/docs/ru/va/chat/title-page)[ ![](https://developers.sber.ru/docs/img/badges/CanvasApp.png)Canvas App ](https://developers.sber.ru/docs/ru/va/canvas/title-page)
...
### q:
q: — (от англ. question — вопрос ) после тега записывается [паттерн](../../nlp/patterns/about-patterns.md). Это локальный тег, переход по которому возможен из **ближайшего родительского** , из **любого дочернего** (в т.ч. из **дочерних соседних стейтов**) или из **соседнего стейта**. Подробнее — в разделе [**Как работают локальные теги**](./declarative-tags.md#kak-rabotayut-lokalnye-tegi).
**Тип значения**
  * multiline string

**Параметры**
  * `fromState` (string) — определяет стейт, из которого смартап может перейти по заданному паттерну.
  * `toState` (string) — определяет стейт, в который перейдет смартап при определении паттерна. При использовании параметра, паттерн, заданный в теге, будет срабатывать только в контексте текущего стейта. При переходе выполняются все реакции заданного стейта (аналогично тегу [go!:](./reaction-tags.md#teg-go12)).
...
**Параметры**
  ...
  * `toState` (string) — определяет стейт, в который перейдет смартап при получении заданного события. При использовании параметра, событие, заданное в теге, будет срабатывать только в контексте текущего стейта. При переходе выполняются все реакции заданного стейта аналогично тегу [go!:](./reaction-tags.md#teg-go12).
  * `onlyThisState` (boolean) — при флаге `true` переход в стейт может быть совершен только из указанного в параметре `fromState` состояния, но не из его дочерних стейтов. По умолчанию `false`.
...
Локальные теги [q:](./declarative-tags.md#q12), [event:](./declarative-tags.md#event12) и [intent:](./intents-tags.md#intent2) срабатывают в следующих случаях:
...
Локальные теги [q:](./declarative-tags.md#q12), [event:](./declarative-tags.md#event12) и [intent:](./intents-tags.md#intent2) НЕ срабатывают в следующих случаях:

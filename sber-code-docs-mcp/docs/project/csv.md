...
Справочники подключаются с помощью тега [`require`](../sa-dsl/tags/declarative-tags.md#require2), в котором требуется указать путь к файлу, а также значения параметров `name` и `var`:
```

...
```
  ...
  1. Нажмите **Начать развертывание** и далее — **Новое развертывание**.

Каждое изменение функции требует новое развертывание.
  1. Выберите тип развертывания - **Веб-приложение**.
  2. При появлении окна с запросом доступа нажмите **Предоставить доступ**.
  3. Нажмите **Начать развертывание**.
  4. Скопируйте URL в поле **Веб-приложение**. Ссылка имеет следующий вид: `https://script.google.com/macros/s/AKf....`.
  5. Добавьте к скопированной ссылке параметры и присвойте значения переменным:
...
```
  ...
  1. На этот же экран добавьте блок **HTTP-запрос**.
  2. В поле URL вставьте ранее полученную ссылку.

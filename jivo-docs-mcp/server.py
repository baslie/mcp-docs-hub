"""
MCP-сервер для семантического поиска по документации Jivo API.
"""
...
def get_category(file_path: Path) -> str:
    ...
    try:
        ...
        if len(parts) == 1:
            # Корневые файлы: определяем категорию по имени
            name = rel_path.stem.lower()
            if name in ("readme", "overview"):
                return "root"
            return name  # widget, webhooks, chat, bot
        return "/".join(parts[:-1])
    except ValueError:
        return "unknown"
...
def search_documentation(
    ...
    """
    Семантический поиск по документации Jivo API.

    Args:
        query: Поисковый запрос (русский/английский)
        category: Фильтр по категории (widget, webhooks, chat, bot, root)
...
def read_full_file(file_path: str) -> str:
    ...
    Args:
        file_path: Путь относительно docs/ (например: widget.md, webhooks.md)
...
def list_categories() -> str:
    ...
    output = ["# Категории документации Jivo API\n"]
    output.append("| Категория | Файлов | Описание |")
    output.append("|-----------|--------|----------|")

    descriptions = {
        "widget": "Widget API — управление виджетом чата",
        "webhooks": "Webhooks API — уведомления о событиях",
        "chat": "Chat API — обмен сообщениями",
        "bot": "Bot API — интеграция чат-ботов",
        "root": "Корневые файлы (README, overview)",
    }

    for cat, count in sorted_cats:
        desc = descriptions.get(cat, "")
        output.append(f"| {cat} | {count} | {desc} |")
...
def find_file_by_name(filename: str) -> str:
    ...
    Args:
        filename: Подстрока для поиска (например: webhook, widget)
    """
...
def list_files_in_category(category: str) -> str:
    ...
    Args:
        category: Название категории (widget, webhooks, chat, bot, root)
    """
    matches = []
...
def search_exact(text: str, category: Optional[str] = None) -> str:
    ...
    Args:
        text: Искомый текст
        category: Опционально, фильтр по категории (widget, webhooks, chat, bot, root)
    """
